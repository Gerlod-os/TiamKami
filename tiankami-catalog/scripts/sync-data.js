...
async function fetchAndParse(url) {
  ...
  return Papa.parse(text, { delimiter: '\t', header: false, skipEmptyLines: false }).data;
}
...
async function sync() {
  ...
  try {
    ...
    fs.writeFileSync(path.join(dataDir, 'games.json'), JSON.stringify(games, null, 2), 'utf-8');
  ...
  try {
    ...
    fs.writeFileSync(path.join(dataDir, 'collections.json'), JSON.stringify(collections, null, 2), 'utf-8');
