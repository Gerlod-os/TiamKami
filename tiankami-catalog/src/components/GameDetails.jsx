import { FaStar, FaClock, FaCalendarAlt, FaYoutube, FaCheckCircle, FaTimesCircle, FaHourglassHalf, FaGamepad } from 'react-icons/fa'
    ...
    <div className="grid md:grid-cols-2 gap-6">
      <div>
        <div className="h-56 bg-gradient-to-br from-accent-purple/20 to-accent-pink/20 rounded-xl flex items-center justify-center mb-4">
          {game.image ? (
            <img src={game.image} alt={game.title} className="w-full h-full object-cover rounded-xl" />
          ) : (
        ...
        <div className="space-y-2 text-sm">
          ...
          <div>
            <span className="text-white/70">Сложность:</span> {game.complexity}/10
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-white/70">Дата выхода:</span> {game.releaseDate}
          </div>
          <div className="flex items-center gap-2">
            <span className="text-white/70">Когда играл:</span> {game.playedDate}
          </div>
          <div className="flex items-center gap-2">
            <span className="text-white/70">Статус:</span> {statusIcons[game.status]} {game.status}
          </div>
          <div>
      ...
      <div>
        ...
        {game.youtube && game.youtube !== 'сылка на плейлист' && (
          <a
            href={game.youtube}
            target="_blank"
        ...
        {game.hasMI && game.hasMI.toLowerCase() === 'true' && (
          <div className="bg-white/5 rounded-xl p-4 mb-4">
            <h3 className="font-heading text-lg mb-2">Многопользовательский интерактив (МИ)</h3>
            {game.miVideo && game.miVideo !== 'Ссылка на выпуск МИ' ? (
              <a
                href={game.miVideo}
                target="_blank"
                rel="noopener noreferrer"
